"""insert_stretch.py [apply] — insert stretching dances (NO music tag, per-dance difficulty) from
_proto/picks_verified7.tsv (name, styleId, difficulty, ytid, dur, title). Per-style-unique slug.
...
rows = []
for line in open("_proto/picks_verified7.tsv", encoding="utf-8"):
    line = line.rstrip("\n")
    if not line: continue
    name, sid, diff, vid, dur, title = line.split("\t")
    rows.append((name, int(sid), int(diff), vid, int(float(dur)), title))
...
for name, sid, diff, vid, dur, title in rows:
    ...
    while sl in existing:
        sl = f"{base}-{i}"; i += 1
    if not APPLY:
        print(f"  DRY + {name:24} style {sid} diff {diff} slug {sl} <- {vid} ({dur}s)")
        continue
    sql = f"""BEGIN;
...
COMMIT;"""
    ...
    print(f"  + {name:24} dance {dance_id} video {vid_db} slug {sl} diff {diff} <- {vid} ({dur}s)")
