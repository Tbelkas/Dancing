...
```json
{
  ...
  "stages": [{ "title": "…", "description": "…",
    "steps": [{ "title": "The jack", "description": "…", "danceSlug": "house-jack" }] }]
}
```
...
3. **Re-resolves every `danceSlug` on every run**, whether or not it rebuilt. A step written
   before its move existed links itself as soon as that move is added to the catalog.
