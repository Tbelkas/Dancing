"""Phase 3 v2: Gap-fill using Ollama knowledge per dance style - improved prompting."""
import json
...
def ollama_list_moves(style_name):
    """Ask Ollama to list dance moves for a style. Uses format:json with an object wrapper."""
    prompt = (
        f"You are a dance expert. List 25 well-known {style_name} dance moves or steps that a beginner should learn. "
        f"Include specific named moves, not generic descriptions. "
    ...
    try:
        with urllib.request.urlopen(req, timeout=120) as r:
            ...
            if isinstance(parsed, dict):
                for key in ("moves", "steps", "dance_moves", "list", "dances", "results"):
...
def main():
    ...
    for style in styles:
        ...
        print(f"  Got {len(moves)} moves from Ollama: {moves[:5]}")

        if not moves:
            print(f"  No moves returned, skipping style")
            continue

        new_moves = [m for m in moves if isinstance(m, str) and m.strip() and m.lower() not in existing_names]
        print(f"  {len(new_moves)} new moves to add")
    ...
    with open("/tmp/phase3b_created.json", "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2)
    print("Saved to /tmp/phase3b_created.json")
    return result
